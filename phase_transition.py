...
def simulate(n: int, d: int, p: float, a: int, measure_for: Optional[list[int]] = None, entanglement_type: EntanglementType = EntanglementType.TRIPARTITE, entanglement_entropy_for: Optional[list[int]] = None) -> float:
    assert(0 <= p <= 1)
...
def main():
    ...
    if args.entanglement_entropy is not None:
        entanglement_entropy_args = {
            ...
            "entanglement_entropy_for": list(map(int, args.entanglement_entropy.split())),
        }

    output_dir = Path(__file__).parent / "output" 
    output = output_dir / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_pt_n{n:02d}_d{d:02d}_a{a}_r{r:02d}"
